...
import copy
import logging
logging.basicConfig(filename='octopus.log', level=logging.WARN)
...
class EthereumSSAEngine(EthereumEmulatorEngine):
    ...
    def findBasicBlockByName(self, bbName):
        for bb in self.basicblocks:
            if bb.name == bbName:
                return bb
        return None
        ...
        while not halt:
            ...
            if instr.name == 'JUMPDEST':
                ...
                if self.current_basicblock.start_offset != instr.offset:
                    ...
                    # Yue
                    next_bb = self.findBasicBlockByName('block_%x'%instr.offset)
                    self.current_basicblock.addNextBBs(next_bb, EDGE_FALLTHROUGH)
                    next_bb.addPrevBBs(self.current_basicblock)
    ...
    def ssa_stack_memory_storage_flow_instruction(self, instr, state, depth):
        ...
        if op == 'POP':
            ...
            if target.offset not in state.instructions_visited:
                ...
                # Yue
                next_bb = self.findBasicBlockByName('block_%x'%target.offset)
                self.current_basicblock.addNextBBs(next_bb, EDGE_UNCONDITIONAL)
                next_bb.addPrevBBs(self.current_basicblock)
            ...
            # Yue
            next_bb = self.findBasicBlockByName('block_%x'%(instr.offset_end + 1))
            self.current_basicblock.addNextBBs(next_bb, EDGE_CONDITIONAL_FALSE)
            next_bb.addPrevBBs(self.current_basicblock)
            ...
            if target.offset not in state.instructions_visited:
                ...
                # Yue
                next_bb = self.findBasicBlockByName('block_%x'%target.offset)
                self.current_basicblock.addNextBBs(next_bb, EDGE_CONDITIONAL_TRUE)
                next_bb.addPrevBBs(self.current_basicblock)
